...
export default function ProductView({ className, reportHandler }) {
  ...
  const changeImgHandler = (current) => {
    setSrc(`http://127.0.0.1:8000${current}`);
  };
  ...
  const filteredImages = selectedSize
    ? productImage.filter((img) => img.size === selectedSize)
    : productImage;
  ...
  <div className="w-full">
    <div className="w-full h-[600px] border border-qgray-border flex justify-center items-center overflow-hidden relative mb-3">
      <img
        src={product && `http://127.0.0.1:8000${product.image}`}
        alt=""
    ...
    <div className="flex gap-2 flex-wrap">
      {filteredImages.map((img) => (
        <div
          onClick={() => changeImgHandler(img.image1)}
          key={img.id}
        ...
        >
          <img
            ...
            alt=""
            className={`w-full h-full object-contain ${src !== `http://127.0.0.1:8000${img.image1}` ? "opacity-50" : ""}`}
          />
        </div>
      ))}
    </div>
        ...
        <div className="product-details w-full mt-10 lg:mt-0">
          {product && (
            <>
              ...
              <div
                data-aos="fade-up"
                className="cart-btn w-full flex items-center mb-[30px]"
              >
                <button
                  className="w-full text-white bg-qyellow font-700 text-[13px] py-[17px] uppercase border border-qyellow hover:bg-white hover:text-qyellow transition duration-[200ms] rounded-[4px]"
                  onClick={() => reportHandler(product, quantity)}
                >
                  Add to Cart
                </button>
              </div>
              <div
                data-aos="fade-up"
                className="wishlist-btn w-full flex items-center mb-[30px]"
              >
                <button
                className="w-full text-white bg-qred font-700 text-[13px] py-[17px] uppercase border border-qyellow hover:bg-white hover:text-qyellow transition duration-[200ms] rounded-[4px]"
